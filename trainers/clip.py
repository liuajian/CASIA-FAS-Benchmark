...
class CLIP(TrainerX):
    ...
    def forward_backward(self, batch):
        XY_R, XY_B, XY_L, XY_P = self.parse_batch_train(batch)
    ...
    def parse_batch_train(self, batch):
        X_R, X_B, X_P, X_L = batch['X_R'].to(self.device), batch['X_B'], batch['X_P'], batch['X_L'].to(self.device)
        Y_R, Y_B, Y_P, Y_L = batch['Y_R'].to(self.device), batch['Y_B'], batch['Y_P'], batch['Y_L'].to(self.device)
        XY_R = torch.cat([X_R, Y_R], dim=0)
        XY_B = torch.cat([X_B, Y_B], dim=0)
        XY_L = torch.cat([X_L, Y_L], dim=0)
        XY_P = X_P + Y_P
        return XY_R, XY_B, XY_L, XY_P
