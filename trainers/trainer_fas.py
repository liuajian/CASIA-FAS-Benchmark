...
class SimpleTrainerIntra(TrainerBase):
    ...
    def after_train(self):
        ...
        if do_test:
            ...
            _, threshold_val = self.test(split="val", threshold=None)
            self.test(split="test", threshold=threshold_val)
    ...
    def after_epoch(self):
        ...
        if do_test and self.cfg.TEST.FINAL_MODEL == "best_val":
            _, threshold_val = self.test(split="val", threshold=None)
            curr_result, _ = self.test(split="test", threshold=threshold_val)
    ...
    @torch.no_grad()
    def test(self, split=None, threshold=None, eval_only=False):
        ...
        results = self.evaluator.evaluate(split, threshold, eval_only)
...
class TrainerXU(SimpleTrainerIntra):
...
class TrainerX(SimpleTrainerIntra):
