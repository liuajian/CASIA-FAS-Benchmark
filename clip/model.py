...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adapter):
        if x.shape[0] == 197:
            x_ = adapter(self.ln_1(x))
        else:
            x_ = 0
        x = x + self.attention(self.ln_1(x)) + x_
        if x.shape[0] == 197:
            x_ = adapter(self.ln_1(x))
        else:
            x_ = 0
        x = x + self.mlp(self.ln_2(x)) + x_
        return x
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        ...
        if width == 768:
            self.adapter = Convpass()  # nn.Identity()
        else:
            self.adapter = None

    def forward(self, x: torch.Tensor):
        for i, blk in enumerate(self.resblocks):
            x = blk(x, self.adapter)
        return x
